  ...
  return (
    <div >
          <Dialog open={dialogOpen} slotProps={{
              backdrop: {
                  sx: {
                      backgroundImage: `url(${hpMediaTravel})`, // Add your image URL here
    ...
    <div style={{ marginTop: '10px' }}>
                      <Stack direction="row" spacing={1}>
                          {countries.map((country) => (
                              <Chip label={country} color="primary" key={country} onClick={() => setSelectedCountry(country)} variant={selectedCountry === country ? "filled" : "outlined"}/>
                          ))}
                          <Chip label="Other" color="primary" onClick={() => {handleOther()}} variant={otherSelected ? "filled" : "outlined"} />
                      </Stack>
    </div>
